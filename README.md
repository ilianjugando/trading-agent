...
Scheduler's job, not the script's:

1. Task Scheduler > Create Task
2. Trigger: e.g. "Daily", repeat task every 30 minutes, for a duration of
   8 hours (covers market hours for stocks; crypto trades 24/7 so consider
   a separate more frequent task with `--pool crypto` if desired)
3. Action: Start a program
   - Program: `C:\Users\ivasc\trading-agent\.venv\Scripts\python.exe`
   - Arguments: `execution\orchestrator.py --mode paper --pool both`
   - Start in: `C:\Users\ivasc\trading-agent`
4. Make sure IB Gateway is set to auto-restart/stay logged in, or the
